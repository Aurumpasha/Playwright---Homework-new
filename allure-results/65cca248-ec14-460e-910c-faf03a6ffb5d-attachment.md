...
Expected: "SUCCESS"
Received: undefined
    at C:\autoTesting\Playwright - Homework new\tests\KMS_item_search.spec.ts:28:40
```
...
> 28 |     expect(sessionResponseBody.status).toBe('SUCCESS');
     |                                        ^ Error: expect(received).toBe(expected) // Object.is equality
  29 |
  30 |     // User data availability
  31 |     expect(sessionResponseBody.data).toHaveProperty('userId');
  32 |     expect(sessionResponseBody.data).toHaveProperty('userFullName');
  33 |     expect(sessionResponseBody.data).toHaveProperty('username');
  34 |     // User information
