...
Received has value: undefined
    at C:\autoTesting\Playwright - Homework new\tests\KMS_item_search.spec.ts:52:37
```
...
```ts
  ...
  35 |     expect(Array.isArray(searchResponseBody.results)).toBeTruthy();
  36 |
  37 |     if (searchResponseBody.results.length > 0) {
  38 |       // Проверяем свойства первого результата
  39 |       const firstResult = searchResponseBody.results[0];
  40 |
  ...
  46 |       expect(firstResult.status).toBe('Online'); // Измените на 'Online' или 'Offline' в зависимости от вашего теста
  47 |     } else {
