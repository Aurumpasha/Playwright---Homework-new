...
```
Error: locator.evaluate: Test timeout of 50000ms exceeded.
Call log:
  - waiting for locator('.cmTreeOnlineStatus button').filter({ hasText: 'General AUTO PA2' })

    at C:\autoTesting\Playwright - Homework new\tests\KMS_item_create.spec.ts:82:49
```
...
- dialog "Message ":
  - text: Message
  - img: 
  - text: This item has been updated by Content Manager at 04/29/2025 18:19:37. Please first update to the latest version You can reload it (all your changes will be lost) or create an item copy.
  - button "Copy the item"
  - button "Reload"
  - button "Cancel"
...
```ts
   ...
   81 |     .filter({ hasText: 'General AUTO PA2' });
>  82 |   const colorOnline = await buttonElementOnline.evaluate((element) => {
  ...
  138 |     .filter({ hasText: 'General AUTO PA3' });
